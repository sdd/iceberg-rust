...
impl TableScan {
    ...
    pub async fn plan_files(&self) -> crate::Result<FileScanTaskStream> {
        ...
        let mut manifest_evaluator_factory_cache: HashMap<i32, ManifestEvaluator> = HashMap::new();
            ...
            for entry in manifest_list.entries() {
                ...
                if let Some(filter) = filter.as_ref() {
                    let manifest_eval_factory = manifest_evaluator_factory_cache
                            ...
                            .or_insert_with_key(|key| Self::create_manifest_eval_factory(key, schema.clone(), table_metadata.clone(), case_sensitive, filter));


                    // reject any manifest files whose partition values don't match the filter.
                    if !manifest_eval_factory.eval(entry)? {
                        continue;
                    }
                }
    ...
    fn create_manifest_eval_factory(
        ...
        let bound_predicate = filter.bind(schema.clone(), case_sensitive).unwrap();
