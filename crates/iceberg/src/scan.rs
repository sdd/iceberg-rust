...
const DEFAULT_CONCURRENCY_LIMIT_MANIFEST_FILES: usize = 10;
const DEFAULT_CONCURRENCY_LIMIT_MANIFEST_ENTRIES: usize = 8;
...
impl Default for TableScanConfig {
    fn default() -> Self {
        Self {
            concurrency_limit_manifest_files: DEFAULT_CONCURRENCY_LIMIT_MANIFEST_FILES,
            concurrency_limit_manifest_entries: DEFAULT_CONCURRENCY_LIMIT_MANIFEST_ENTRIES,
        }
    }
}
...
impl TableScan {
    ...
    pub async fn plan_files(&self) -> Result<FileScanTaskStream> {
        ...
        let (manifest_entry_ctx_tx, manifest_entry_ctx_rx) =
            channel(DEFAULT_CONCURRENCY_LIMIT_MANIFEST_FILES);
        // used to stream the results back to the caller
        let (file_scan_task_tx, file_scan_task_rx) =
            channel(DEFAULT_CONCURRENCY_LIMIT_MANIFEST_ENTRIES);
        ...
        futures::stream::iter(manifest_file_contexts)
            .try_for_each_concurrent(DEFAULT_CONCURRENCY_LIMIT_MANIFEST_FILES, |ctx| async move {
        ...
        manifest_entry_ctx_rx
            ...
            .try_for_each_concurrent(
                DEFAULT_CONCURRENCY_LIMIT_MANIFEST_ENTRIES,
